...
import { Plus, Trash2, Star, ChevronDown, ChevronUp, MoreVertical, Layers, Copy, FolderPlus, Menu, Settings, Files, Pencil, Gift, X, GripVertical, Search, Bookmark, Edit as EditIcon } from 'lucide-react';
...
export default function DishesTab({ onNavigateToPizzas }) {
  ...
  const [filterType, setFilterType] = useState('all'); // all, highlight, new, popular
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      ...
      setDishFormData(prev => ({ ...prev, image: file_url }));
    }
  };
  ...
  const updateComplementOptionImage = async (groupId, optionId, file) => {
    ...
    const newOptions = (group.options || []).map(opt =>
      opt.id === optionId ? { ...opt, image: file_url } : opt
    );
  ...
  const handleGroupSettingsSave = () => {
    ...
    // Atualizar o grupo global (max_selection)
    updateComplementGroupMutation.mutate({
      id: editingGroup.id,
      data: {
    ...
    // Atualizar o is_required no relacionamento do prato
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <ReorderModal
        ...
        onSave={(updates) => {
          ...
          // Save options order for each group
